# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 14:57:53 2020
...
def main():
    ...
    while(1):
        ...
        Binv=matInverse1(B) # Inverse of B    
        X=np.matmul(Binv,b)
        Z=np.matmul(CB,X)
...
def getInitialBVindicesAndNBVindices(A,n,m):
    ...
    NBVindices=[] # Indices of columns in A to be put in intial B matrix   
    for i in range(0,n):
        zeroes=0
        ones=0
        column=np.copy(A[:,[i]])
...
# Calculating inverse using GJR
def matInverse1(B):
    n,m=B.shape
    I=np.identity(n)
    if(n==m):
        for k in range(0,n):            
            a=np.copy(B[:,[k]]) # Get column from B                      
            v=np.matmul(I,a)            
            wlist=[]
            for i in range(0,n):
                if(v[k]!=0 and i!=k):
                    wlist.append(-1*(v[i]/v[k]))
                if(v[k]!=0 and i==k):
                    wlist.append(1/v[k])
            w=np.array(wlist)            
            b=np.copy(I[[k],:])            
            for x in range(0,n):
                I[k][x]=0            
            I=np.add(I,np.kron(w,b)) # kron is used for tensor product of w and b          
        return I # Inverse of B
